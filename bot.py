...
class WorkBot:
    def __init__(self):
        ...
        # Фразы для ответных постов (работодатель помечает, что работа выполнена)
        self.completion_phrases = [
            "уже сделали, спасибо",
            "работа выполнена, все четка",
            ...
            "выполнили, спасибо",
            "сделали быстро, все четка",
            "готово, работали как надо",
            "выполнили качественно",
            "сделали, спасибо за работу",
            "готово, все четка",
            "выполнили быстро и хорошо",
            "сделали, работали качественно",
            "готово, спасибо",
            "выполнили, все как надо",
            "сделали быстро, спасибо",
            "готово, качественно поработали",
            "выполнили, работали хорошо"
        ]
        
        # Дополнительные детали для ответных постов (тупые подарки)
        self.completion_details = [
            "еще и 15 гвоздей подарили",
            "еще и флешку 5гб дали",
            "еще и старую батарейку оставили",
            "еще и ржавый болт подарили",
            "еще и сломанную отвертку дали",
            "еще и пустую банку из-под краски оставили",
    ...
    def schedule_messages(self):
        """Планирует отправку сообщений 10 раз в день (2 голосовых, 8 текстовых) + ответные сообщения через 30 минут"""
        # 7:00 утра - ГОЛОСОВОЕ СООБЩЕНИЕ
        schedule.every().day.at("07:00").do(self.send_voice_message_sync)
        # 7:30 утра - ответное сообщение на голосовое
        schedule.every().day.at("07:30").do(self.send_completion_message_sync)
        # 9:00 утра - текстовое
        schedule.every().day.at("09:00").do(self.send_message_sync)
        # 9:30 утра - ответное сообщение на текстовое
        schedule.every().day.at("09:30").do(self.send_completion_message_sync)
        # 11:00 утра - текстовое
        schedule.every().day.at("11:00").do(self.send_message_sync)
        # 11:30 утра - ответное сообщение на текстовое
        schedule.every().day.at("11:30").do(self.send_completion_message_sync)
        # 13:00 дня - текстовое
        schedule.every().day.at("13:00").do(self.send_message_sync)
        # 13:30 дня - ответное сообщение на текстовое
        schedule.every().day.at("13:30").do(self.send_completion_message_sync)
        # 15:00 дня - текстовое
        schedule.every().day.at("15:00").do(self.send_message_sync)
        # 15:30 дня - ответное сообщение на текстовое
        schedule.every().day.at("15:30").do(self.send_completion_message_sync)
        # 17:00 дня - ГОЛОСОВОЕ СООБЩЕНИЕ
        schedule.every().day.at("17:00").do(self.send_voice_message_sync)
        # 17:30 дня - ответное сообщение на голосовое
        schedule.every().day.at("17:30").do(self.send_completion_message_sync)
        # 19:00 вечера - текстовое
        schedule.every().day.at("19:00").do(self.send_message_sync)
        # 19:30 вечера - ответное сообщение на текстовое
        schedule.every().day.at("19:30").do(self.send_completion_message_sync)
        # 21:00 вечера - текстовое
        schedule.every().day.at("21:00").do(self.send_message_sync)
        # 21:30 вечера - ответное сообщение на текстовое
        schedule.every().day.at("21:30").do(self.send_completion_message_sync)
        # 23:00 вечера - текстовое
        schedule.every().day.at("23:00").do(self.send_message_sync)
        # 23:30 вечера - ответное сообщение на текстовое
        schedule.every().day.at("23:30").do(self.send_completion_message_sync)
        # 1:00 ночи - текстовое
        schedule.every().day.at("01:00").do(self.send_message_sync)
        # 1:30 ночи - ответное сообщение на текстовое
        schedule.every().day.at("01:30").do(self.send_completion_message_sync)
        
        logger.info("Расписание сообщений настроено: 07:00 (🎤), 07:30 (✅), 09:00, 09:30 (✅), 11:00, 11:30 (✅), 13:00, 13:30 (✅), 15:00, 15:30 (✅), 17:00 (🎤), 17:30 (✅), 19:00, 19:30 (✅), 21:00, 21:30 (✅), 23:00, 23:30 (✅), 01:00, 01:30 (✅)")
