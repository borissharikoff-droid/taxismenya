...
class WorkBot:
    def __init__(self):
        ...
        # Фразы для ответных постов (что работа уже выполнена)
        self.completion_phrases = [
            "уже забрали, братя. сделали",
            "все четка сделали заебись, подарили еще куртку кожану",
            "забрал, братан. все готово, еще и чай с печеньками дали",
            "сделал, земляк. хозяин доволен, еще и такси оплатил",
            "все готово, друг. работали как надо, еще и обед накрыли",
            "забрал, товарищ. сделали быстро, еще и инструменты подарили",
            "уже сделано, братя. хозяин в шоке от качества, еще и премию дал",
            "все четка, земляк. работали не покладая рук, еще и ужин накрыли",
            "готово, друг. хозяин доволен, еще и куртку кожану подарил",
            "сделали, товарищ. все как надо, еще и чай с булочками дали",
            "уже забрали, братя. работали как звери, еще и обед борщ накрыли",
            "все готово, земляк. хозяин в восторге, еще и такси домой оплатил",
            "сделал, друг. работали качественно, еще и материалы оставили",
            "забрал, товарищ. все четка, еще и чай с печеньками дали",
            "уже сделано, братя. хозяин доволен, еще и куртку кожану подарил",
            "все готово, земляк. работали не покладая рук, еще и ужин накрыли",
            "сделали, друг. качественно поработали, еще и инструменты оставили",
            "забрал, товарищ. хозяин в шоке, еще и премию дал",
            "уже готово, братя. работали как надо, еще и обед накрыли",
            "все четка, земляк. хозяин доволен, еще и такси оплатил"
        ]
        
        # Дополнительные детали для ответных постов
        self.completion_details = [
            "еще и чай с печеньками дали",
            "еще и куртку кожану подарили", 
            "еще и обед борщ накрыли",
            "еще и такси домой оплатили",
            "еще и материалы оставили",
            "еще и инструменты подарили",
            "еще и премию дали",
            "еще и ужин накрыли",
            "еще и чай с булочками дали",
            "еще и куртку кожану оставили",
            "еще и обед накрыли",
            "еще и такси оплатили",
            "еще и материалы подарили",
            "еще и инструменты оставили",
            "еще и премию дали",
            "еще и ужин накрыли",
            "еще и чай с печеньками дали",
            "еще и куртку кожану подарили",
    ...
    def convert_to_personal_voice_style(self, text):
        """Конвертирует текст под дагестанский говор с правильным русским произношением"""
        # Словарь замен для дагестанского говора (сохраняем правильное произношение)
        voice_replacements = {
            # Характерные звуковые замены дагестанского говора (мягкие)
            "ч": "ч", "щ": "щ", "ц": "ц", "ж": "ж",  # Сохраняем правильные звуки
            "ы": "ы", "э": "э", "ю": "ю", "я": "я", "ё": "ё",  # Сохраняем правильные звуки
            "ь": "ь", "ъ": "ъ",  # Сохраняем мягкие и твердые знаки
            ...
            # Характерные фразы дагестанского говора
            "₽": " руб",
            ",": ", ну,",
            ".": ", понял?",
            "!": ", да!",
        ...
        # Характерные вставки дагестанского говора + мат
        ...
        # Специальные замены для дагестанского говора + мат
        mat_replacements = {
            "плохо": "блядски плохо",
            ...
            "легко": "бляяя легко",
            "работа": "работа, братан",
            "деньги": "деньги, земляк",
            "время": "время, друг",
            "место": "место, товарищ"
        }
        ...
        # Добавляем дагестанские обращения или мат в начало
        if random.random() < 0.4:  # 40% шанс
            start_words = ["ну, ", "братан, ", "земляк, ", "друг, ", "товарищ, ", "бля, ", "епты бля, ", "ебать бля, ", "блядь, "]
            start_word = random.choice(start_words)
            result = start_word + result
        
        # Добавляем дагестанские обращения или мат в конец
        if random.random() < 0.25:
            end_words = [", братан", ", земляк", ", друг", ", товарищ", ", бля", ", епты бля", ", ебать бля", ", блядь", ", епта"]
    ...
    def improve_russian_pronunciation(self, text):
        """Улучшает русское произношение для ElevenLabs"""
    ...
    def generate_completion_message(self):
        """Генерирует ответное сообщение о том, что работа уже выполнена"""
        # Выбираем основную фразу
        main_phrase = random.choice(self.completion_phrases)
        
        # 50% шанс добавить дополнительную деталь
        if random.random() < 0.5:
    ...
    def generate_voice_message(self, text):
        ...
        try:
            # Конвертируем текст в дагестанский говор
            ...
            logger.info(f"Текст для озвучки (дагестанский говор): {personal_text}")
            ...
            if self.elevenlabs_client and ELEVENLABS_VOICE_ID:
                ...
                audio = self.elevenlabs_client.text_to_speech.convert(
                    ...
                    voice_settings={
                        "stability": 0.7,  # Увеличиваем стабильность для четкого произношения
                        "similarity_boost": 0.9,  # Максимальное сходство с оригиналом
                        "style": 0.2,  # Уменьшаем стиль для более естественного звучания
    ...
    def schedule_messages(self):
        """Планирует отправку сообщений 15 раз в день (2 голосовых, 8 текстовых, 5 ответных)"""
        # 7:00 утра - ГОЛОСОВОЕ СООБЩЕНИЕ
        schedule.every().day.at("07:00").do(self.send_voice_message_sync)
        # 8:00 утра - ответное сообщение
        schedule.every().day.at("08:00").do(self.send_completion_message_sync)
        # 9:00 утра - текстовое
        schedule.every().day.at("09:00").do(self.send_message_sync)
        # 10:00 утра - ответное сообщение
        schedule.every().day.at("10:00").do(self.send_completion_message_sync)
        ...
        # 14:00 дня - ответное сообщение
        schedule.every().day.at("14:00").do(self.send_completion_message_sync)
        # 15:00 дня - текстовое
        schedule.every().day.at("15:00").do(self.send_message_sync)
        # 16:00 дня - ответное сообщение
        schedule.every().day.at("16:00").do(self.send_completion_message_sync)
        # 17:00 дня - ГОЛОСОВОЕ СООБЩЕНИЕ
        schedule.every().day.at("17:00").do(self.send_voice_message_sync)
        # 18:00 вечера - ответное сообщение
        schedule.every().day.at("18:00").do(self.send_completion_message_sync)
        ...
        logger.info("Расписание сообщений настроено: 07:00 (🎤), 08:00 (✅), 09:00, 10:00 (✅), 11:00, 13:00, 14:00 (✅), 15:00, 16:00 (✅), 17:00 (🎤), 18:00 (✅), 19:00, 21:00, 23:00, 01:00")
    ...
    async def start_bot(self):
        ...
        try:
            ...
            logger.info("Бот готов к работе! Сообщения будут отправляться в 07:00 (🎤), 08:00 (✅), 09:00, 10:00 (✅), 11:00, 13:00, 14:00 (✅), 15:00, 16:00 (✅), 17:00 (🎤), 18:00 (✅), 19:00, 21:00, 23:00, 01:00")
